...
export async function runImport(
  ...
  if (targetType === "transactions") {
    ...
    // type
    if (!record.type) {
      const debitAcc = record.debit_account_id || "";
      const creditAcc = record.credit_account_id || "";
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (targetType === "transactions") {
        ...
        // Конвертируем даты в Date для Prisma
        if (typeof record.date === "string") {
          record.date = new Date(record.date + "T00:00:00.000Z");
        }
        if (record.accrual_date && typeof record.accrual_date === "string") {
        ...
        record.company_id = companyId;
        record.amount_rub = record.amount_rub ?? record.amount;
        // Определяем type по счетам
        if (!record.type) {
          const debitAcc =
            record.debit_account_id || record.debit_account || "";
          const creditAcc =
            record.credit_account_id || record.credit_account || "";
          if (creditAcc.startsWith("acc-in-")) {
            record.type = "income";
          } else if (
            debitAcc.startsWith("acc-bank-") &&
