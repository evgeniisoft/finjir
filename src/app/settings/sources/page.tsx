...
export default function DataSourcesPage() {
      ...
      if (lines.length > 0) {
        ...
        for (const h of headers) {
          ...
          if (l.includes("дат")) auto[h] = "date";
            ...
            l.includes("контрагент") ||
            l.includes("поставщик") ||
            l.includes("покупател")
          )
            auto[h] = "counterparty";
  ...
  const handleDeleteSource = async (source: any, e: React.MouseEvent) => {
    ...
    if (
      !confirm(
        `Удалить источник "${source.name}"? Связанные маппинги тоже будут удалены.`,
      )
    )
      return;
    try {
      // Удаляем маппинги источника
      const relatedMappings = mappings.filter((m) => m.source_id === source.id);
      for (const m of relatedMappings) {
        await api.delete("DataMappings", m.id);
      }
    ...
    try {
      ...
      const payload = {
        ...
        defaults: JSON.stringify({
          currency: sourceForm.default_currency,
          record_type: sourceForm.record_type,
        }),
        transforms: JSON.stringify({
          date: "parse_date",
          amount: "parse_float",
        }),
        dedup_key: JSON.stringify([
          "date",
          "amount",
          "company_id",
          "description",
  ...
  const handleRollback = async (batch_id: string) => {
    if (!confirm("Откатить импорт? Все записи этой партии будут удалены."))
      return;
    try {
  ...
  const targetFields = selectedSource
    ? getTargetFields(selectedSource.target_type)
    : [];
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setSourceForm({
                    ...sourceForm,
                    target_type: e.target.value as ImportTargetType,
                  })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setSourceForm({ ...sourceForm, company_id: e.target.value })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
              >
          ...
          <div className="space-y-2">
            {sources.map((s) => (
              <div
                ...
                className={`p-4 border rounded-lg cursor-pointer ${selectedSource?.id === s.id ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-gray-50"}`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-gray-900">{s.name}</p>
                    <p className="text-sm text-gray-500">
                      {s.type} • {s.target_type}
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <button
                      onClick={(e) => handleRenameSource(s, e)}
                      className="text-blue-600 hover:text-blue-800 text-sm font-medium"
                    >
                      Изменить
                    </button>
                    <button
                      onClick={(e) => handleDeleteSource(s, e)}
      ...
      {selectedSource && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <div className="mb-4">
            ...
            />
            {fileName && (
              <p className="text-sm text-gray-500 mt-1">Загружен: {fileName}</p>
            )}
          </div>
                    ...
                    <thead>
                      <tr>
                        {fileHeaders.map((h) => (
                          <th
                            key={h}
                            className="px-2 py-1 text-left text-xs bg-gray-50"
                          >
                            {h}
                          </th>
                        ))}
              ...
              {importResult && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                  <p className="text-blue-800">
                    Импорт: создано {importResult.imported}, пропущено{" "}
                    {importResult.skipped}, ошибок{" "}
                  ...
                  {importResult.errors?.length > 0 && (
                    <details className="mt-2">
                      ...
                      <pre className="text-xs text-red-500 mt-1 whitespace-pre-wrap max-h-40 overflow-auto">
                        {importResult.errors.slice(0, 20).join("\n")}
                      </pre>
                    </details>
                  )}
                  <button
